...
|---|---|---|---|
| Brute force / credential stuffing | Scripted guessing against the auth endpoints | Token-bucket rate limiting keyed per client on every auth endpoint (login, register, refresh, forgot/reset-password, verify/resend-verification); see [`RateLimitFilter`](server/src/main/java/com/krino/backend/security/RateLimitFilter.java), [`Bucket4jRateLimiter`](server/src/main/java/com/krino/backend/security/Bucket4jRateLimiter.java) | ◐ buckets are in-process (single instance) and there is no per-account lockout, so a distributed low-and-slow guess against one account is not throttled by account |
...
| User enumeration | Response differences reveal whether an email is registered | Forgot-password and resend-verification return the same response whether or not the account exists; login only reveals the unverified-email state *after* the password was proven correct; see [`PasswordResetService`](server/src/main/java/com/krino/backend/service/PasswordResetService.java), [`EmailVerificationService`](server/src/main/java/com/krino/backend/service/EmailVerificationService.java), [`AuthenticationService`](server/src/main/java/com/krino/backend/service/AuthenticationService.java) | ◐ registration still returns 409 for a taken email (an accepted trade-off, mitigated by per-IP rate limiting) |
...
| Horizontal-scaling gaps | Second replica gets fresh rate-limit buckets; no shared token denylist | Documented single-instance assumption | ✗ Redis-backed buckets + denylist planned before scaling out |
